...
class FreshnessFirstReplacePendingByKeyRecordProcessingRuntimeTest {
    ...
    @Test
    fun `when equal byte array key is already queued then newer record replaces it`() = runBlocking {
        val firstStarted = CompletableDeferred<Unit>()
        val releaseFirst = CompletableDeferred<Unit>()
        val processedOffsets = CopyOnWriteArrayList<Long>()
        val metrics = RecordingMetrics<ByteArray, String>()
        val runtime = byteArrayFreshnessByKeyRuntime(
            metrics = metrics,
            handler = KafkaRecordHandler { record ->
                if (record.offset() == 1L) {
                    firstStarted.complete(Unit)
        ...
        runtime.start { throw it }
        assertTrue(runtime.tryEmit(byteArrayKeyRecord(offset = 1L, key = byteArrayOf(9))))
        withTimeout(2_000) { firstStarted.await() }
        assertTrue(runtime.tryEmit(byteArrayKeyRecord(offset = 2L, key = byteArrayOf(1, 2))))
        assertTrue(runtime.tryEmit(byteArrayKeyRecord(offset = 3L, key = byteArrayOf(1, 2))))
    ...
    private fun byteArrayFreshnessByKeyRuntime(
        metrics: ConsumerMetrics<ByteArray, String>,
        handler: KafkaRecordHandler<ByteArray, String>
    ): FreshnessFirstReplacePendingByKeyRecordProcessingRuntime<ByteArray, String> =
        FreshnessFirstReplacePendingByKeyRecordProcessingRuntime(
            workerConcurrency = 1,
            workChannelCapacity = 4,
    ...
    private fun byteArrayKeyRecord(offset: Long, key: ByteArray): ConsumerRecord<ByteArray, String> =
        ConsumerRecord("topic-a", 0, offset, key, "value-$offset")
}
