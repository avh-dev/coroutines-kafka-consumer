...
def window_audit(
    ...
    cached_windows = audit.get("measurement_windows")
    if isinstance(cached_windows, list):
        for cached in cached_windows:
            if isinstance(cached, dict) and str(cached.get("name") or "") == name:
                return cached
    warnings.append(f"Measurement window {name} is absent from the persisted audit summary")
    return {}
...
def analyze_experiment(
    ...
    for target in experiment_summary.get("targets", []):
            ...
            for window in configured_measurement_windows(target_load_test):
                ...
                if window_duration > 0 and window_start + window_duration <= target_load_duration:
                    try:
                        ...
                        audit_document = window_audit(audit, window, warnings)
                        result.update({
                            "delivery": (
        ...
        execution = (
            "COMPLETED"
            if str(status.get("status") or "").lower() == "completed" and int(target.get("exit_code", 1)) == 0
            else str(status.get("status") or "unknown").upper()
        )
