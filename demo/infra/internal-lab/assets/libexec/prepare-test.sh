...
kafka_runtime_memory() {
  ...
  if [[ "${value}" =~ ^([1-9][0-9]*)(Mi|Gi)$ ]]; then
    printf '%s%s\n' "${BASH_REMATCH[1]}" "${BASH_REMATCH[2],,}" | tr -d i
    return
  fi
  echo "Kafka memory values must use Mi or Gi: ${value}" >&2
  exit 1
}
...
LAB_KAFKA_MEMORY_RUNTIME="$(kafka_runtime_memory "${LAB_KAFKA_MEMORY_PER_BROKER}")"
LAB_KAFKA_HEAP_RUNTIME="$(kafka_runtime_memory "${LAB_KAFKA_HEAP_PER_BROKER}")"
if [[ "${PROCESSING_ENABLED}" != "true" && "${PROCESSING_ENABLED}" != "false" ]]; then
...
if [[ -n "${KAFKA_CONSUMER_MAX_POLL_INTERVAL_MS:-}" ]] && ! [[ "${KAFKA_CONSUMER_MAX_POLL_INTERVAL_MS}" =~ ^[1-9][0-9]*$ ]]; then
  echo "KAFKA_CONSUMER_MAX_POLL_INTERVAL_MS must be a positive integer after overrides: ${KAFKA_CONSUMER_MAX_POLL_INTERVAL_MS}" >&2
  exit 1
fi
CONSUMER_SIZE_ENV_NAMES=(
  KAFKA_CONSUMER_FETCH_MAX_BYTES KAFKA_CONSUMER_MAX_PARTITION_FETCH_BYTES
  ORDER_KAFKA_CONSUMER_FETCH_MIN_BYTES ORDER_KAFKA_CONSUMER_FETCH_MAX_WAIT_MS ORDER_KAFKA_CONSUMER_MAX_POLL_RECORDS ORDER_KAFKA_CONSUMER_FETCH_MAX_BYTES ORDER_KAFKA_CONSUMER_MAX_PARTITION_FETCH_BYTES
  BATCH_KAFKA_CONSUMER_FETCH_MIN_BYTES BATCH_KAFKA_CONSUMER_FETCH_MAX_WAIT_MS BATCH_KAFKA_CONSUMER_MAX_POLL_RECORDS BATCH_KAFKA_CONSUMER_FETCH_MAX_BYTES BATCH_KAFKA_CONSUMER_MAX_PARTITION_FETCH_BYTES
  TELEMETRY_KAFKA_CONSUMER_FETCH_MIN_BYTES TELEMETRY_KAFKA_CONSUMER_FETCH_MAX_WAIT_MS TELEMETRY_KAFKA_CONSUMER_MAX_POLL_RECORDS TELEMETRY_KAFKA_CONSUMER_FETCH_MAX_BYTES TELEMETRY_KAFKA_CONSUMER_MAX_PARTITION_FETCH_BYTES
)
for env_name in "${CONSUMER_SIZE_ENV_NAMES[@]}"; do
  env_value="${!env_name:-}"
  if [[ -n "${env_value}" ]] && ! [[ "${env_value}" =~ ^[1-9][0-9]*$ ]]; then
    echo "${env_name} must be a positive integer after overrides: ${env_value}" >&2
    exit 1
  fi
done
...
LAB_KAFKA_BROKER_COUNT="${LAB_KAFKA_BROKER_COUNT:-}" \
LAB_KAFKA_REPLICATION_FACTOR="${LAB_KAFKA_REPLICATION_FACTOR:-}" \
LAB_KAFKA_MIN_INSYNC_REPLICAS="${LAB_KAFKA_MIN_INSYNC_REPLICAS:-}" \
LAB_KAFKA_CPU_PER_BROKER="${LAB_KAFKA_CPU_PER_BROKER:-}" \
LAB_KAFKA_MEMORY_PER_BROKER="${LAB_KAFKA_MEMORY_PER_BROKER:-}" \
LAB_KAFKA_HEAP_PER_BROKER="${LAB_KAFKA_HEAP_PER_BROKER:-}" \
...
EXPERIMENT_TARGET_NAME='${EXPERIMENT_TARGET_NAME:-}'
...
KAFKA_CONSUMER_MAX_POLL_INTERVAL_MS='${KAFKA_CONSUMER_MAX_POLL_INTERVAL_MS:-1800000}'
...
echo "  kafka_brokers=${LAB_KAFKA_BROKER_COUNT:-}"
echo "  kafka_replication_factor=${LAB_KAFKA_REPLICATION_FACTOR:-}"
echo "  kafka_min_insync_replicas=${LAB_KAFKA_MIN_INSYNC_REPLICAS:-}"
...
echo "  model_sync_http_client=${MODEL_SYNC_HTTP_CLIENT:-ARMERIA}"
