...
PRODUCER_CONFIG_STEPS_JSON="${PRODUCER_CONFIG_STEPS_JSON:-[]}"
...
write_run_metadata() {
  ...
  export CHAOS_STEPS_JSON DIAGNOSTIC_STEPS_JSON PRODUCER_CONFIG_STEPS_JSON RUN_PACKET_CAPTURE_DIR RUN_EVENTS_FILE LOAD_TEST_LOG_FILE
...
metadata = {
    ...
    "load_test": {
        ...
        "producer_config_steps": json.loads(env("PRODUCER_CONFIG_STEPS_JSON", "[]")),
    },
...
nohup python3 "${LAB_ROOT}/helpers/run-load-test-with-events.py" \
  --log-file "${LOAD_TEST_LOG_FILE}" -- "${LOAD_TEST_BIN}" >/dev/null 2>&1 &
