...
CONSUMER_DRAIN_IDLE_SECONDS="${CONSUMER_DRAIN_IDLE_SECONDS:-60}"
TEST_DEFINITION=""
DEPLOYMENT_PROFILE=""
RUN_PROFILE=""
...
if [ -z "${LAB_KAFKA_IMPLEMENTATION}" ]; then
  if [ ! -t 0 ]; then
    ...
    LAB_KAFKA_IMPLEMENTATION="$(select_value "Kafka broker implementation" "${CURRENT_KAFKA_IMPLEMENTATION}" apache-kafka redpanda)"
  fi
fi
...
if [ "${EXPERIMENT_GRAFANA_ANNOTATIONS_ENABLED}" != "true" ] && [ "${EXPERIMENT_GRAFANA_ANNOTATIONS_ENABLED}" != "false" ]; then
  echo "EXPERIMENT_GRAFANA_ANNOTATIONS_ENABLED must be true or false: ${EXPERIMENT_GRAFANA_ANNOTATIONS_ENABLED}" >&2
  exit 1
fi
if [ "${EXPERIMENT_GRAFANA_RUN_ANNOTATIONS_ENABLED}" != "true" ] && [ "${EXPERIMENT_GRAFANA_RUN_ANNOTATIONS_ENABLED}" != "false" ]; then
  echo "EXPERIMENT_GRAFANA_RUN_ANNOTATIONS_ENABLED must be true or false: ${EXPERIMENT_GRAFANA_RUN_ANNOTATIONS_ENABLED}" >&2
  exit 1
fi
for drain_setting in CONSUMER_DRAIN_TIMEOUT_SECONDS CONSUMER_DRAIN_STABLE_SECONDS CONSUMER_DRAIN_POLL_SECONDS CONSUMER_DRAIN_IDLE_SECONDS; do
  drain_value="${!drain_setting:-}"
  if ! [[ "${drain_value}" =~ ^[1-9][0-9]*$ ]]; then
    echo "${drain_setting} must be a positive integer: ${drain_value}" >&2
    exit 1
  fi
done
...
if [ "${RUN_PREPARE}" -eq 1 ]; then
  ...
  AUDIT_RUN_ID="${RUN_ID}" KAFKA_TOPIC_METADATA_FILE="${RESULTS_DIR}/runs/${RUN_ID}/diagnostics/kafka-metadata.json" "${LAB_ROOT}/libexec/prepare-test.sh" "${PREPARE_ARGS[@]}"
  if [ -f "${CURRENT_DEPLOYMENT_PATH}" ]; then
    MODEL_SYNC_HTTP_CLIENT="$(sed -n "s/^MODEL_SYNC_HTTP_CLIENT='\([^']*\)'$/\1/p" "${CURRENT_DEPLOYMENT_PATH}" | tail -n 1)"
  fi
  MODEL_SYNC_HTTP_CLIENT="${MODEL_SYNC_HTTP_CLIENT:-ARMERIA}"
else
...
# Materialize load-generator defaults in evidence while retaining topic-level
# overrides as independent values that inherit from these shared settings.
KAFKA_PRODUCER_LINGER_MS="${KAFKA_PRODUCER_LINGER_MS:-20}"
KAFKA_PRODUCER_BATCH_SIZE="${KAFKA_PRODUCER_BATCH_SIZE:-65536}"
KAFKA_PRODUCER_COMPRESSION_TYPE="${KAFKA_PRODUCER_COMPRESSION_TYPE:-lz4}"
...
write_run_metadata() {
  ...
  export TELEMETRY_KAFKA_PRODUCER_LINGER_MS TELEMETRY_KAFKA_PRODUCER_BATCH_SIZE TELEMETRY_KAFKA_PRODUCER_COMPRESSION_TYPE TELEMETRY_KAFKA_PRODUCER_BUFFER_MEMORY
  export KAFKA_CONSUMER_FETCH_MIN_BYTES KAFKA_CONSUMER_FETCH_MAX_WAIT_MS KAFKA_CONSUMER_MAX_POLL_RECORDS KAFKA_CONSUMER_MAX_POLL_INTERVAL_MS KAFKA_CONSUMER_FETCH_MAX_BYTES KAFKA_CONSUMER_MAX_PARTITION_FETCH_BYTES
  export ORDER_KAFKA_CONSUMER_FETCH_MIN_BYTES ORDER_KAFKA_CONSUMER_FETCH_MAX_WAIT_MS ORDER_KAFKA_CONSUMER_MAX_POLL_RECORDS ORDER_KAFKA_CONSUMER_FETCH_MAX_BYTES ORDER_KAFKA_CONSUMER_MAX_PARTITION_FETCH_BYTES
  ...
  export EXPERIMENT_NAME EXPERIMENT_TARGET_NAME EXPERIMENT_TARGET_INDEX EXPERIMENT_TARGET_TOTAL EXPERIMENT_RUN_ANNOTATION_LABEL
...
metadata = {
    ...
    "drain_wait": {
        ...
        "idle_seconds": env_int("CONSUMER_DRAIN_IDLE_SECONDS"),
    },
    "application": {
    ...
    "kafka": {
        ...
        "brokers": env_int("LAB_KAFKA_BROKER_COUNT"),
        "replication_factor": env_int("LAB_KAFKA_REPLICATION_FACTOR"),
        "min_insync_replicas": env_int("LAB_KAFKA_MIN_INSYNC_REPLICAS"),
        "resources": {
            "cpu_per_broker": float(env("LAB_KAFKA_CPU_PER_BROKER")) if env("LAB_KAFKA_CPU_PER_BROKER") else None,
            "memory_per_broker": env("LAB_KAFKA_MEMORY_PER_BROKER"),
            "heap_per_broker": env("LAB_KAFKA_HEAP_PER_BROKER"),
        },
        ...
        "consumer": {
            ...
            "fetch_max_wait_ms": env_int("KAFKA_CONSUMER_FETCH_MAX_WAIT_MS"),
            "max_poll_records": env_int("KAFKA_CONSUMER_MAX_POLL_RECORDS"),
            "max_poll_interval_ms": env_int("KAFKA_CONSUMER_MAX_POLL_INTERVAL_MS"),
            "fetch_max_bytes": env_int("KAFKA_CONSUMER_FETCH_MAX_BYTES"),
            "max_partition_fetch_bytes": env_int("KAFKA_CONSUMER_MAX_PARTITION_FETCH_BYTES"),
        },
        "topic_consumers": {
            topic.lower(): {
                "fetch_min_bytes": env_int(f"{topic}_KAFKA_CONSUMER_FETCH_MIN_BYTES"),
                "fetch_max_wait_ms": env_int(f"{topic}_KAFKA_CONSUMER_FETCH_MAX_WAIT_MS"),
                "max_poll_records": env_int(f"{topic}_KAFKA_CONSUMER_MAX_POLL_RECORDS"),
                "fetch_max_bytes": env_int(f"{topic}_KAFKA_CONSUMER_FETCH_MAX_BYTES"),
                "max_partition_fetch_bytes": env_int(f"{topic}_KAFKA_CONSUMER_MAX_PARTITION_FETCH_BYTES"),
            }
    ...
    "load_test": {
        ...
        "producer_capacity_tps": {
            "order": env_int("ORDER_TPS_PER_PRODUCER"),
            "batch": env_int("BATCH_TPS_PER_PRODUCER"),
            "telemetry": env_int("CAULDRON_TELEMETRY_TPS_PER_PRODUCER"),
        },
        "metrics_port": env_int("LOAD_TEST_METRICS_PORT"),
        ...
        "telemetry_publish_interval_seconds": env_int("TELEMETRY_PUBLISH_INTERVAL_SECONDS"),
        "kafka_producer": {
            "linger_ms": env_int("KAFKA_PRODUCER_LINGER_MS"),
            "batch_size": env_int("KAFKA_PRODUCER_BATCH_SIZE"),
            "compression_type": env("KAFKA_PRODUCER_COMPRESSION_TYPE"),
            "buffer_memory": env_int("KAFKA_PRODUCER_BUFFER_MEMORY"),
        },
        "topic_kafka_producers": {
            topic.lower(): {
                "linger_ms": env_int(f"{topic}_KAFKA_PRODUCER_LINGER_MS"),
                "batch_size": env_int(f"{topic}_KAFKA_PRODUCER_BATCH_SIZE"),
                "compression_type": env(f"{topic}_KAFKA_PRODUCER_COMPRESSION_TYPE"),
                "buffer_memory": env_int(f"{topic}_KAFKA_PRODUCER_BUFFER_MEMORY"),
            }
            for topic in ("ORDER", "BATCH", "TELEMETRY")
        },
    },
    ...
    "experiment_events": {
        "file": env("RUN_EVENTS_FILE"),
        "load_test_log": env("LOAD_TEST_LOG_FILE"),
        "grafana_annotations_enabled": env_bool("EXPERIMENT_GRAFANA_ANNOTATIONS_ENABLED"),
        "grafana_run_annotations_enabled": env_bool("EXPERIMENT_GRAFANA_RUN_ANNOTATIONS_ENABLED"),
    },
    "packet_captures": {
...
if ! TEST_RUN_ID="${RUN_ID}" \
  ...
  EXPERIMENT_GRAFANA_RUN_ANNOTATIONS_ENABLED="${EXPERIMENT_GRAFANA_RUN_ANNOTATIONS_ENABLED}" \
  EXPERIMENT_GRAFANA_URL="${EXPERIMENT_GRAFANA_URL:-http://127.0.0.1:3000}" \
  python3 "${LAB_ROOT}/helpers/publish-run-start.py" --metadata "${RUN_METADATA_FILE}"; then
  echo "Warning: failed to record the run-start annotation." >&2
fi
