...
REGION="${1:-us-east-1}"
ENVIRONMENT="${2:-dev}"
TEST_DEFINITION_PATH="${3:-demo/infra/shared/test-definitions/ckc-baseline.yaml}"
INSTANCE_ID="${4:-}"
...
COMMANDS_FILE="$(mktemp "${TEMP_DIR}/ckc-run-test-ssm.XXXXXX.json")"
cat > "${COMMANDS_FILE}" <<EOF
{
  "commands": [
    ...
    "if [ ! -f \\"\\\${RUNNER_REPO_DIR}/demo/infra/aws/runner-internal/run-test.sh\\" ]; then echo Runner assets are expected at \\\${RUNNER_REPO_DIR} >&2; exit 1; fi",
    "mkdir -p /opt/ckc-runner/reports",
    "LOG_FILE=/opt/ckc-runner/reports/test-launch-\\$(date -u +%Y%m%dT%H%M%SZ).log",
    "cd \\"\\\${RUNNER_REPO_DIR}\\"",
    "CKC_RUNNER_REPO_DIR=\\"\\\${RUNNER_REPO_DIR}\\" nohup ./demo/infra/aws/runner-internal/run-test.sh ${REGION} ${ENVIRONMENT} \\"\\\${TEST_DEFINITION_PATH}\\" > \\"\\\${LOG_FILE}\\" 2>&1 < /dev/null &",
    "echo started=true",
