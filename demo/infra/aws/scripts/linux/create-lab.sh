...
DEFAULT_TEST_DEFINITION_PATH="demo/infra/shared/test-definitions/ckc-baseline.yaml"
ARG4="${4:-}"
...
COMMANDS_FILE="$(mktemp "${TEMP_DIR}/ckc-create-lab-ssm.XXXXXX.json")"
cat > "${COMMANDS_FILE}" <<EOF
{
  "commands": [
    ...
    "cd \\"\\\${RUNNER_REPO_DIR}\\"",
    "CKC_RUNNER_REPO_DIR=\\"\\\${RUNNER_REPO_DIR}\\" ./demo/infra/aws/runner-internal/create-lab.sh ${REGION} ${ENVIRONMENT} ${PROFILE_NAME} ${TEST_DEFINITION_PATH}"
  ]
}
EOF
