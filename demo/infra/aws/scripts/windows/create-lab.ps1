param(
    ...
    [string]$TestDefinitionPath = "demo/infra/shared/test-definitions/ckc-baseline.yaml"
)
...
if ($PSVersionTable.PSEdition -ne "Core") {
    ...
    if (-not $pwsh) {
        throw "create-lab.ps1 requires PowerShell 7. Install pwsh and rerun the script."
    }
...
$tempFile = Join-Path $tempDir ("ckc-create-lab-ssm.{0}.json" -f [System.IO.Path]::GetRandomFileName())
try {
    $commands = @{
        commands = @(
            "set -euo pipefail",
            ...
            "cd `"`${RUNNER_REPO_DIR}`"",
            "CKC_RUNNER_REPO_DIR=`"`${RUNNER_REPO_DIR}`" ./demo/infra/aws/runner-internal/create-lab.sh $Region $Environment $ProfileName $TestDefinitionPath"
        )
