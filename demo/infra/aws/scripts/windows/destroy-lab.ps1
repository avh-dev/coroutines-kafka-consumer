...
$tempFile = Join-Path $tempDir ("ckc-destroy-lab-ssm.{0}.json" -f [System.IO.Path]::GetRandomFileName())
try {
    $commands = @{
        commands = @(
            "set -euo pipefail",
            ...
            "cd `"`${RUNNER_REPO_DIR}`"",
            "CKC_RUNNER_REPO_DIR=`"`${RUNNER_REPO_DIR}`" ./demo/infra/aws/runner-internal/destroy-lab.sh $Region $Environment $ProfileName"
        )
