...
python3 "${REPO_ROOT}/demo/infra/shared/test-orchestration/flush-redis.py" \
  --host "${redis_service}.ckc-app.svc.cluster.local"

python3 "${REPO_ROOT}/demo/infra/shared/test-orchestration/prepare-kafka-topics.py" \
...
if [ "${SKIP_BUILD}" != "true" ]; then
  ...
  docker build -f demo/ckc-demo/Dockerfile -t ckc-local/demo:latest demo/ckc-demo
  docker build -f demo/ckc-demo-stubs/Dockerfile -t ckc-local/demo-stubs:latest demo/ckc-demo-stubs
  docker build -f demo/ckc-demo-load-test/Dockerfile -t ckc-local/load-test:latest demo/ckc-demo-load-test
