...
try {
    ...
    Invoke-Checked python @(
        (Join-Path $repoRoot "demo\infra\shared\test-orchestration\flush-redis.py"),
        "--host", "$redisService.ckc-app.svc.cluster.local"
    )

    Invoke-Checked python @(
        (Join-Path $repoRoot "demo\infra\shared\test-orchestration\prepare-kafka-topics.py"),
    ...
    if (-not $SkipBuild) {
        ...
        Invoke-Checked docker @("build", "-f", "demo/ckc-demo/Dockerfile", "-t", "ckc-local/demo:latest", "demo/ckc-demo")
        Invoke-Checked docker @("build", "-f", "demo/ckc-demo-stubs/Dockerfile", "-t", "ckc-local/demo-stubs:latest", "demo/ckc-demo-stubs")
        Invoke-Checked docker @("build", "-f", "demo/ckc-demo-load-test/Dockerfile", "-t", "ckc-local/load-test:latest", "demo/ckc-demo-load-test")
    ...
    kubectl -n ckc-observability create configmap ckc-grafana-dashboard-provider --from-file=ckc.yml=demo/infra/shared/grafana/provisioning/dashboards/ckc.yml --dry-run=client -o yaml | kubectl apply -f -
    kubectl -n ckc-observability create configmap ckc-grafana-dashboard --from-file=ckc-overview.json=demo/infra/shared/grafana/dashboards/ckc-overview.json --dry-run=client -o yaml | kubectl apply -f -
