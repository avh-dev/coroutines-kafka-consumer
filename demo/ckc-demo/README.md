...
- `PRODUCER_CONFIG_STEPS_JSON` schedules runtime producer-pool replacements. Each ordered JSON item uses `atSeconds`, a topic (`order`, `batch`, `telemetry`, or `all`), and at least one of `lingerMs`, `batchSize`, `compressionType`, or `bufferMemory`; the load test emits `CKC_EXPERIMENT_EVENT` records at the actual start and completion time.
