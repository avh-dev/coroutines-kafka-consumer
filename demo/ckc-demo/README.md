...
- `FRESHNESS_FIRST_MAX_RECORD_AGE_SECONDS=10` limits record age for freshness-first external adapter handling. Stale records are acknowledged without running demo business logic, but still emit dropped audit records and `demo.ckc.record.dropped{reason="stale_age"}` metrics.
