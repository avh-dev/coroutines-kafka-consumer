...
<a id="infra-129"></a>
### INFRA-129 - Preserve AWS workload logs

_Date: 2026-09-02_

Export Kubernetes logs from the start of target orchestration so application startup and deployment diagnostics are retained even when the workload starts later.
...
<a id="infra-151"></a>
### INFRA-151 - Use the installed TShark Kafka topic field
...
<a id="infra-193"></a>
### INFRA-193 - Shorten the single-target smoke experiment
...
<a id="demo-97"></a>
### DEMO-97 - Make the demo-stub request timeout configurable
...
<a id="infra-206"></a>
### INFRA-206 - Exercise two measurement windows in smoke

_Date: 2026-09-22_

Extend smoke to two minutes, use an 80-second max-load measurement interval, and add a 40-second interval after stub degradation ends.
Leave enough time for several Prometheus samples in both windows while keeping the existing chaos and packet-capture schedule.
Verification: all 37 experiment-orchestration and 76 internal-lab tests pass; the canonical smoke validates and whitespace validation passes. No smoke run was launched automatically.

<a id="infra-207"></a>
### INFRA-207 - Show duration brackets on workload timelines

_Date: 2026-09-22_

Render every duration-based measurement, chaos, and diagnostic event with an explicit horizontal start-to-end arrow in a dedicated band between the plot and all cards.
Order arrow lanes chronologically from top to bottom, continue each start guide through its arrow to the matching card, and stop the end guide at the arrow.
Keep cards in the opposite bottom-to-top chronological order.
Keep instantaneous event markers unchanged so duration ranges remain a distinct visual layer.
Verification: all 76 internal-lab and 14 result-bundle tests pass; a current two-window smoke model renders one bounded, in-canvas arrow lane for every duration event, and Python and whitespace validation pass.
...
<a id="infra-208"></a>
### INFRA-208 - Restore native consumer observability

_Date: 2026-09-23_

Restore native Confluent Parallel Consumer metrics and their dedicated Grafana section.
Keep the full section in live dashboards while omitting it from evidence bundles whose run metadata contains no CPC targets.
Correct the restored encoding and offset-payload views to match the metrics emitted by the current Parallel Consumer version.
...
Verification: 106 internal-lab tests, Bash and Python syntax checks, Gradle agent staging and demo compilation, and whitespace checks passed. A real optilab update completed without building Thread Stats; an immediate unchanged `lab.sh up` transferred no files and completed in 0.78 seconds. Both application deployments were returned to zero replicas afterward.
