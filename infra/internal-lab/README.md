...
`infra/internal-lab` runs the CKC demo on a dedicated Linux laptop with low overhead.
...
```sh
./infra/internal-lab/scripts/install-lab.sh <LAB_IP>
```

The installer:

- writes `.internal-lab/lab.env`
- copies `infra/internal-lab/assets` to `/opt/ckc-internal-lab/assets` on the lab host
...
```sh
./infra/internal-lab/scripts/build-load-images.sh
```
...
```sh
./infra/internal-lab/scripts/set-test.sh
```
...
```sh
./infra/internal-lab/scripts/prepare-test.sh
```

You can still pass an explicit definition when needed:

```sh
./infra/internal-lab/scripts/prepare-test.sh ckc-baseline-local
```
...
```sh
./infra/internal-lab/scripts/run-test.sh
```

You can still pass an explicit definition when needed:

```sh
./infra/internal-lab/scripts/run-test.sh ckc-baseline-local
```
...
```sh
helm upgrade --install ckc-demo infra/shared/helm/demo \
  --kubeconfig "$KUBECONFIG" \
  --namespace ckc-perf \
  -f infra/internal-lab/assets/config/demo-values.yaml \
  -f infra/shared/helm/demo/profiles/ckc-hpa.yaml
...
helm upgrade --install ckc-demo infra/shared/helm/demo \
  ...
  --namespace ckc-perf \
  -f infra/internal-lab/assets/config/demo-values.yaml \
  -f infra/shared/helm/demo/profiles/ckc-local-baseline.yaml \
