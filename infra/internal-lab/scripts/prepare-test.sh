...
helm upgrade --install ckc-demo-stubs "${REPO_ROOT}/infra/shared/helm/demo-stubs" \
  --namespace ckc-perf \
  -f "${REPO_ROOT}/infra/internal-lab/assets/config/demo-stubs-values.yaml" \
  -f "${REPO_ROOT}/infra/shared/helm/demo-stubs/profiles/${STUBS_PROFILE}.yaml"

helm upgrade --install ckc-demo "${REPO_ROOT}/infra/shared/helm/demo" \
  --namespace ckc-perf \
  -f "${REPO_ROOT}/infra/internal-lab/assets/config/demo-values.yaml" \
  -f "${REPO_ROOT}/infra/shared/helm/demo/profiles/${APP_PROFILE}.yaml"
